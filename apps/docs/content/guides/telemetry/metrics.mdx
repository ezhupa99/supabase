...
For this guide, we will be using [Grafana](https://grafana.com/docs/grafana/latest/introduction/). If you use [Grafana Cloud](https://grafana.com/grafana/), refer to the [Grafana Cloud configuration section](/docs/guides/telemetry/metrics#grafana-cloud).
...
## Using the metrics endpoint in production for Grafana Cloud [#grafana-cloud]
...
- Gather the following details in `Grafana Data Source settings` to be used in the next section.
  - `URL` (will be of the format `https://UNIQUE_IDENTIFIER.grafana.net/api/prom`)
  - `User` / `Password`

### Adding a Prometheus data source

- In the left-hand menu, select `Connections`, `Data sources` and click `Add new data source`.
- Search for `Prometheus` and select it.
- Under **Name** use `prometheus` (if you use a different name, take note of it for adding the dashboard).
- Fill in the following details based on the section above.
...
- If you have name the Prometheus data source in the previous step as `prometheus`:
  - Paste the contents of the file (or upload the file).
- If you have not named your Prometheus data source `prometheus`
  - Find all occurences of `"uid": "prometheus"` and replace `prometheus` with your data source name
- Click `Load` and the dashboard will load from the project specified in your Prometheus job.
